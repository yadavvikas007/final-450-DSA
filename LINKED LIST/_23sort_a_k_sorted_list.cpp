...
struct Node* sortAKSortedDLL(struct Node* head, int k)          //TIME: O(NlogK)  SPACE: O(k)
{ 
	// if list is empty 
	if (head == NULL) 
		return head; 
	...
	while (!pq.empty()) { 
		...
		if (newHead == NULL) { 
			...
			last = newHead;                 //last is on the last element of sorted list so far 
		} 
